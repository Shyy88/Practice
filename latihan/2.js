...
function makanSkuy(data) {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (sales[i] > 20_000_000 && votes[i] > 100) {
            category = 'ThreeStars';
        } else if (sales[i] >= 10_000_000 && votes[i] >= 50) {
            category = 'TwoStars'
        } else if (sales[i] >= 5000000 && sales[i] <= 9_999_999 && votes[i] >= 25 && votes[i] <= 49) {
            category = 'OneHalfStar' // nambah onehalfstar
        }
        if (!result[category]) {
            result[category] = [];
        }
